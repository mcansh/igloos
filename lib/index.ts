...
const urls: Array<string> = [`https://thewhitehorseinn.checkfront.com`];

const IglooChecker = async (_req: VercelRequest, res: VercelResponse) => {
  const now = new Date();
  ...
  try {
    ...
    const datesAvailable = payloads.reduce(
      (acc: Array<{ [key: string]: Array<Date> }>, payload, index) => {
        ...
        if (!url) {
          ...
          Sentry.captureException(new Error(`No url found for index ${index}`));
          return acc;
        }
    ...
    if (messages.length > 0) {
      ...
      const promises = env.PHONE_NUMBERS.map((phone) =>
